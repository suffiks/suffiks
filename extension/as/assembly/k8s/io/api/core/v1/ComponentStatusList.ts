...
export class ComponentStatusList {
  static encode(message: ComponentStatusList, writer: Writer): void {
    ...
    for (let i: i32 = 0; i < items.length; ++i) {
      writer.uint32(18);
      writer.fork();
      ComponentStatus.encode(items[i], writer);
      writer.ldelim();
    }
  }

  static decode(reader: Reader, length: i32): ComponentStatusList {
    const end: usize = length < 0 ? reader.end : reader.ptr + length;
    const message = new ComponentStatusList();
    ...
    while (reader.ptr < end) {
      ...
      switch (tag >>> 3) {
        ...
        case 2:
          message.items.push(ComponentStatus.decode(reader, reader.uint32()));
          break;
