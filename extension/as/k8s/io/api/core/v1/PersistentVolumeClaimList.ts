...
export class PersistentVolumeClaimList {
  static encode(message: PersistentVolumeClaimList, writer: Writer): void {
    ...
    for (let i: i32 = 0; i < items.length; ++i) {
      writer.uint32(18);
      writer.fork();
      PersistentVolumeClaim.encode(items[i], writer);
      writer.ldelim();
    }
  }

  static decode(reader: Reader, length: i32): PersistentVolumeClaimList {
    const end: usize = length < 0 ? reader.end : reader.ptr + length;
    const message = new PersistentVolumeClaimList();
