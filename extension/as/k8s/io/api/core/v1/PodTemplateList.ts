...
export class PodTemplateList {
  static encode(message: PodTemplateList, writer: Writer): void {
    ...
    for (let i: i32 = 0; i < items.length; ++i) {
      writer.uint32(18);
      writer.fork();
      PodTemplate.encode(items[i], writer);
      writer.ldelim();
    }
  }

  static decode(reader: Reader, length: i32): PodTemplateList {
    const end: usize = length < 0 ? reader.end : reader.ptr + length;
    const message = new PodTemplateList();
    ...
    while (reader.ptr < end) {
      ...
      switch (tag >>> 3) {
        ...
        case 2:
          message.items.push(PodTemplate.decode(reader, reader.uint32()));
          break;
